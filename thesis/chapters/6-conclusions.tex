...
While reviewing the runtime's register classification, I also found that the check identifying floating-point registers matched any register name starting with the letter \texttt{f}, which would have misclassified the frame pointer (\texttt{fp}) as a floating-point register. This was never triggered in practice, since \texttt{fp} never reaches that dispatch as a move operand, but the check has since been tightened to match the exact float-register naming convention instead.
